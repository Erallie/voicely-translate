...
`languages` オプションは任意です。言語を指定した場合、その言語が翻訳セッションで使用されます：
...
言語を指定せずに `/join` を使用した場合、Voicely Translate はサーバーのデフォルト翻訳言語を使用します。

サーバーにデフォルト言語が設定されていない場合、Bot は言語を指定するか、サーバー管理者にデフォルト言語を設定してもらうよう案内します。

接続すると、Voicely Translate は文字起こしと翻訳を**そのボイスチャンネルのサイドチャット**に投稿します。

## デフォルト翻訳言語

サーバー管理者は、`languages` オプションを指定せずに `/join` が実行された場合に Voicely Translate が使用する言語を設定できます。

使用方法：

`/defaultlanguages languages:<言語タグ>`

例：

`/defaultlanguages languages:en,ja`

英語と日本語をサーバーのデフォルト翻訳言語として設定します。

複数の言語はカンマで区切ってください：

`/defaultlanguages languages:en,ja,es,fr`

これらのデフォルト設定は Discord サーバーごとに保存され、サーバー管理者が変更するまで有効です。

`/join` で言語を直接指定すると、その翻訳セッションではサーバーのデフォルト設定よりも指定した言語が優先されます。保存されているデフォルト設定自体は変更されません。

このコマンドを使用するには**管理者**権限が必要です。
...
新しいサーバーには現在**50無料トライアルクレジット**が付与されます。これは約**1,000件の5秒間の音声メッセージ**に相当しますが、実際の使用量は異なる場合があります。
...
サーバーが使用するクレジット数は、会話量や有効になっている翻訳言語の数によって異なります。以下の表は、一般的な5秒間の音声メッセージを基準とした月間使用量の目安です。実際の使用量は異なる場合があります。

| サーバーの使用量 | 5秒間の音声メッセージ数/月（目安） | クレジット/月 | 月額の目安 |
| --- | ---: | ---: | ---: |
| ほぼ未使用 / テスト | 0～1,000 | 0～50 | $0～$0.50 |
| 少ない | 1,000～3,000 | 50～150 | $0.50～$1.50 |
| 一般的な利用 | 3,000～10,000 | 150～500 | $1.50～$5.00 |
| 多い | 10,000～30,000 | 500～1,500 | $5.00～$15.00 |
| 非常に多い | 30,000～100,000 | 1,500～5,000 | $15.00～$50.00 |
