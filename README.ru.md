# Voicely Translate

[![Invite Me](https://img.shields.io/badge/Invite%20Me-7965c7?style=for-the-badge)](https://discord.com/oauth2/authorize?client_id=1535789654974930964)
[![App Directory](https://img.shields.io/badge/App%20Directory-615ccc?style=for-the-badge)](https://discord.com/application-directory/1535789654974930964)
[![Privacy Policy](https://img.shields.io/badge/Privacy%20Policy-cc7a7c?style=for-the-badge)](https://github.com/Erallie/voicely-translate/blob/main/legal/privacy-policy.md)
<br>
[![Our Discord](https://img.shields.io/discord/1102582171207741480?style=for-the-badge&logo=discord&logoColor=ffffff&label=Our%20Discord&color=5865F2)](https://discord.gozarproductions.com)
[![Our Other Projects](https://img.shields.io/badge/Our%20Other%20Projects-%E2%9D%A4-563294?style=for-the-badge&logo=data%3Aimage%2Fwebp%3Bbase64%2CUklGRu4DAABXRUJQVlA4WAoAAAAQAAAAHwAAHwAAQUxQSGABAAABgFtbm5volyZTA%2BtibzK2H0w5sDkmhe3GmxrwxGg0839r%2FvkkOogIBW7bKB0c4%2BARYihzIqfd6dfO%2B%2B3XtHsq4jJhlIvcDRcgNB%2FeieQETorBHgghRtUYqwDs%2B4U4IpcvUB%2BVUPSK54uEnTwsUJoar2DeMpzLxQpeG5DH8lxyyfLivVYAwPBbkWdOBg3qFlqiLy679iHy9UDKMZRXmYxpCcusayTHG01K%2FEtatYWuj7oI9hL4BxsxVwhoP2mlAJJ%2BuuAflc6%2BEUCQTCX9EV87xBR2H75NxLZSpWiwzqdIm7ZO7uB3oEgZKbD9Nt3EmHweEPH1t1GNsZUbKeisiwjyTm5fA3SO1yCrADZXrV2PZQJPL1tjN4%2BxUL9ie1mJobzOnDwSx6ILiF%2FW%2BTUR4tcHx0UaV75JXC1a4g6Ky5dLcTSuy9q4HhTieF64Hy1A3GHB8gLLK2e92feuqnbfPK8IVlA4IGgCAACwDQCdASogACAAPk0cjEQioaEb%2BqwAKATEtgBOl7v9V3sHcA2wG4A3gD0APLP9jX9n%2F2jmqv5AZRh7J%2BN2fOx22iE%2F4TUsecFmY%2BSf1r%2BAP%2BTfzT%2FXdIB7KX7MtdIGr1A8H0jmrrfZvqButwOaYcLWYNRq5QgAAP7%2F%2FmIMpiVNn67QXpM1rrDmRS8Nr%2F6dhD%2Bq5e%2BM%2BAtUP1%2FxOj85Ol5y3ebjz%2BpHoOf%2FWW8a%2F2ojUaKVDkVqof%2Bv4f0f6ud8i58wusz%2Fyrj%2F%2BwnM3q0769dvK%2F%2BQe04xL49tkb9t6ylCqqezZtZGuGLJ%2F5iUrPqdYc%2F8VbYZfP%2FOpZP%2F4X4q%2BqS4gPOxzdINOe5PGv%2F0TS%2FJRf4LlFrFkrWtxlS8n40grV%2BKUu%2FiwzdQzImvwH81FxL1bZyTSsrYwMku1Pk9StTtWNjSR8ZWEYBH9eTn%2FvBERii5XaWOPJ%2FFVXtVQGbv%2BFRW5jbo9tfFDu%2BDHHf8LbgUd%2F8W8Id1AehBtRNsLQWbADmvF1QJU8x5tw%2FtTUwIoSaa%2F2jkcvyVHkAsb2qoIh1KF1pPdae%2BZaqjydy6nUa9agjrDk1G4pMhEUhH%2BV%2FIUe49MjhR%2FuxyFmwQ8dDogMyQ%2BdcSBa56Lwt1wyJ%2F22%2F5O98r6q6wiM63HyaYONd36W7br%2F0%2F6y2DZ3irAddj%2FRxntvr%2FbbChSYXAfEbO%2FD0G%2FFbMFqTHypodt9T6dAx%2BUjJYfHzFf%2FM3Ec%2FAtwbjc2gka6urN1MlSLb2VTS9Q5r8fkDzxZz6vu1OYUPUB1UFMIhYGvMATbxxoTmVhvpovzAc%2F8nbOjw3wAAA)](https://github.com/Erallie)
[![Donate](https://img.shields.io/badge/Donate-%24-563294?style=for-the-badge&logo=ko-fi&logoColor=FFFFFF&color=FF6433)](https://www.ko-fi.com/GozarProductions)

---

## Руководство пользователя

Voicely Translate — Discord-бот, который автоматически распознаёт речь в голосовых каналах и переводит её на несколько языков. Каждый говорящий обрабатывается отдельно, поэтому поддерживается одновременная речь нескольких людей, а результаты публикуются в боковом чате голосового канала.

## Возможности
* Автоматическая транскрипция и перевод на несколько языков.
* Несколько языков перевода одновременно.
* Добавление и удаление языков во время сеанса.
* Теги BCP 47 без жёсткого списка разрешённых языков.
* Автоматический выход из пустого канала.
* Совместная работа с [**Voicely Text**](https://discord.com/application-directory/1290741552158609419) для озвучивания переводов.

## Установка и запуск
Добавьте бота по [ссылке](https://discord.com/oauth2/authorize?client_id=1535789654974930964), войдите в обычный голосовой канал и используйте `/join languages:en,ja`. Несколько тегов разделяются запятыми. Результаты публикуются в **боковом чате этого голосового канала**.

`/languages` показывает распространённые теги: `en`, `ja`, `es`, `fr`, `de`, `ko`, `zh`, `haw`. Поддерживаются варианты вроде `pt-BR` и `zh-TW`. Список не является ограничением.

## Изменение языков
`/add languages:fr,ko` — добавить; `/remove languages:en,fr` — удалить; `/active` — показать активные; `/leave` — остановить и выйти.

## Таймаут пустого канала
Администратор может использовать `/timeout seconds:60`. По умолчанию — **30 секунд**.

## Voicely Credits
**100 Voicely Credits = 1,00 USD.** Новые серверы сейчас получают **50 бесплатных пробных кредитов**. Транскрипция и перевод расходуют кредиты; больше языков может увеличить расход.

`/balance` показывает доступные, пробные и купленные кредиты. `/usage` показывает общий расход API, транскрипцию, перевод и общий объём покупок. Кредиты принадлежат **Discord-серверу**.

Администраторы используют `/topup`, чтобы получить уникальный код сервера и пополнить баланс через Ko-fi. **1,00 USD добавляет 100 кредитов.** Укажите код точно в сообщении платежа, затем используйте `/balance` или `/join`.

## Как это работает
1. Бот получает аудио каждого говорящего.
2. Распознаёт речь на фактически используемом языке.
3. Переводит на активные языки.
4. Публикует оригинал и переводы в боковом чате.

Короткие паузы определяют конец реплики. Невербальные звуки и отдельные звуки-размышления могут игнорироваться.

## Команды
`/join` начать · `/add` добавить · `/remove` удалить · `/active` активные языки · `/languages` теги · `/leave` выйти · `/balance` баланс · `/usage` расход · `/topup` пополнение (админ) · `/timeout` таймаут (админ).

## Качество и конфиденциальность
Говорите чётко и избегайте громкого фонового шума. Автоматическая система может ошибаться в именах, сленге и неоднозначной речи. Для работы бот обрабатывает голосовое аудио; участники должны знать, что во время присутствия бота разговор транскрибируется.

## Поддержка
Если у вас возникли проблемы с ботом или вы хотите предложить функцию, создайте [issue](https://github.com/Erallie/voicely-translate/issues), и я постараюсь помочь!
